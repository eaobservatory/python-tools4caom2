...
class test_delayed_error_warning(unittest.TestCase):
    def setUp(self):
        """
        Create a temp directory and delayed_error_warning object.  The temporary 
        directory must be shared amongst several subprocesses, so we cannot use
        mkdtemp.
        """
        self.tmpdir = tempfile.mktemp(dir='/tmp')
        os.mkdir(self.tmpdir)
        self.logfile = os.path.join(self.tmpdir, 'test_delay_error_warning.log')
    ...
    def testZerolength(self):
        ...
        self.assertTrue(len(self.dew.warnings) == 0,
                        'wrong number of warnings: ' + str(len(self.dew.warnings)))
    ...
    def testFitsverify(self):
        ...
        self.assertTrue(self.dew.error_count() == 1,
                        'wrong number of errors: ' + str(self.dew.error_count()))
